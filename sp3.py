...
class WebApplication(server.App):
    ...
    def getData(self, params):
        ...
        df = frame[frame['ind']==province_id]
        df = df[df['year'] == with_year]
        df = df[df['week'] >= with_week]
        df = df[df['week'] <= to_week]
        if index=='VHI':
            df = df.drop(['ind', 'SMN', 'SMT', 'VCI', 'TCI','year'], axis=1)
        if index=='VCI':
            df = df.drop(['ind', 'SMN', 'SMT', 'VHI', 'TCI','year'], axis=1)
        if index=='TCI':
            df = df.drop(['ind', 'SMN', 'SMT', 'VCI', 'VHI','year'], axis=1)
        return df
